    ...
    <header className={`bg-blue w-full font-body z-10 transition-all ease-linear duration-300 ${headerClass ? 'fixed top-0' : '-top-32'}`}>
      <div className="container mx-auto flex flex-wrap px-4 py-2 items-center">
        ...
        <nav className={isActive ? 'flex flex-col items-center absolute top-full right-0 p-4 bg-blue w-full h-screen text-center' : 'ml-auto hidden lg:flex lg:gap-4 flex-wrap items-center text-base justify-center'}>
          <NavLink to={'/'} className="capitalize inline-block py-4 text-white hover:text-orange">home</NavLink>
          <NavLink to={'/about-us'} className="capitalize inline-block py-4 text-white hover:text-orange">about us</NavLink>
          <NavLink to={'/services'} className="capitalize inline-block py-4 text-white hover:text-orange group">
            <span>services </span>
            <FaChevronDown className="inline-block group-hover:fill-orange w-3 h-3" />
          </NavLink>
          <NavLink to={'/contact-us'} className="capitalize inline-block py-4 text-white hover:text-orange">contact us</NavLink>
          <NavLink to={'/free-consultation'} className="p-2 border-2 inline-block border-white text-white hover:border-orange hover:bg-orange">Free Consultation</NavLink>
        </nav>
      </div>
    </header>
  );
